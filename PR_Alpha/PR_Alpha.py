...
#Constants
#Min and Max IP in length (no lower than 1, no higher than 255)
MIN_IP = 1
MAX_IP = 256
#Windows response if a host is up is zero, hence why this var = 0. Only change if Windows response is no longer 0#Made by Cristian Di Bartolomeo
...
while True:
        ...
        if cho == "N" or cho == "n":
                break
WIN_HOST_UP_RESPONSE = 0
#Socket timeout - how long it should check in seconds
SOCK_TIMEOUT = 2
...
def PingSweep(ip):
	upHost = []
	#Checks for last digit in IP, where x = last digit. Will loop and scan using Windows Batch
	for x in range(MIN_IP, MAX_IP):
		resp = os.system("ping -c 1 " +ip+"."+str(x) +" | find \"TTL=\"")
		if (resp == WIN_HOST_UP_RESPONSE):
			print(ip+"."+str(x)+" is up.")
			upHost.append(ip+"."+str(x)+" is up.")
	return upHost
...
while True:
		...
		if opt == 1:
				...
				if error == 1:
					...
					break
				while True:
					#Checks if y exceeds the list of ports to check. Uses socket library to see if port is closed or open.
						if(y < leng):
								soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
								soc.settimeout(SOCK_TIMEOUT)
								if soc.connect_ex((ip, portlist[y])):
		...
		if opt == 2:
				...
				if error == 1:
					...
					break
				for num in range(int(i1), int(i2)+1):
					#Port scanning using the socket library
						soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
						soc.settimeout(SOCK_TIMEOUT)
						if soc.connect_ex((ip, val)):
								if con == 1:
